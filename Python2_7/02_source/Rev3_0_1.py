...
rc_path = '/media/ubuntu/2236c044-00df-38be-2cd0-472d4e62f054/' + str(datetime.datetime.now()) + '/'
if not os.path.exists(rc_path):
    	    os.makedirs(rc_path)

log_file= path + '/04_log/error_log_' + str(datetime.datetime.now()) + '.log'
...
_record_time_threshold_ = datetime.timedelta(seconds=10)
...
Out = cv2.VideoWriter(rc_path + str(datetime.datetime.now()) + '.mp4',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
...
class AlarmCalculate():
    ...
    def close_eye_detection(self, time_window, EAR_window, EAR_Normal):
        ...
        if self.mar_flag == False:
			...
			logging.info('Fatigue alarm goes off. \n')
