...
rc_path = '/media/ubuntu/Data/' + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '/'
if not os.path.exists(rc_path):
	os.makedirs(rc_path)

log_file= path + '/04_log/error_log_' + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.log'
...
Out = cv2.VideoWriter(rc_path + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.mp4',fourcc, 10, (frame_width,frame_height))

font          = cv2.FONT_HERSHEY_SIMPLEX
text_location = (400,450)
fontScale     = 0.5
fontColor     = (128, 128, 0)
lineType      = 2
...
logging.info('EAR_Normal: ' + str(EAR_Normal))
...
class EARCalculate():
	...
	#Cap = cv2.VideoCapture(1)
	    ...
	    if (datetime.datetime.now() - record_start_time) > _record_time_threshold_:
		Out.release()
		record_start_time = datetime.datetime.now()
		Out = cv2.VideoWriter(rc_path + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.mp4',fourcc, 10, (frame_width,frame_height))
	    ...
	    cv2.putText(frame, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'), text_location, font, fontScale, fontColor, lineType)
		...
		# Calculate & Handle Alarm
                ...
                if clear_flag:
		    logging.info('EAR: ' + str(EAR_raw))
		    logging.info('MAR: ' + str(MAR) + '\n')
